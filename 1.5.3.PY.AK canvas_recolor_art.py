...
#     The changes from canvas_circle_art are  
#     the new recolor() event handler, lines 129-139
#     the binding of that handler and giving canvas focus in lines 144-148
#     and updating the UI instructions in lines 91-94
# (c) 2013 PLTW
#
# 
#####
...
class ColorSlider(Tkinter.Scale): # ColorSlider is a subclass of Tkinter.Scale
    ...
    def __init__(self, parent, myLabel, model_intvar, editor, canvas):
        ...
        def slider_changed(new_val):
            ...
            global red_intvar, green_intvar, blue_intvar # the sliders' data
            global shapes # the canvas' data
...
def recolor(event):
    ...
    for shape in shapes:
        ...
        x = (x1+x2) / 2 # find center of circle
        y = (y1+y2) / 2 
        r = (x2-x1) / 2 # find radius of circle
        # find distance from center to mouse click
        d = ((x-event.x)**2 + (y-event.y)**2)**.5  # Pythagorean theorem
        if d < r:
            canvas.itemconfig(shape, fill=tk_color_string)
        
# Subscribe handlers to the Button-1 and ButtonRelease-1 events
canvas.bind('<Button-1>', down)
canvas.bind('<ButtonRelease-1>', up)
# Subscribe handler to the r key
canvas.bind('<r>', recolor)
# Only the one widget with 'focus' gets keyboard events. 
# Place this focus on canvas.
canv as.focus_set() 
