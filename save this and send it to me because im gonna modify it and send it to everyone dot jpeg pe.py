...
def getDirectory():
    top.destroy()
    a = Tkinter.Tk()
    label = Tkinter.Label(a, text= 'Oh shitass waddup')
    label.pack()
    btn = Tkinter.Button(text = "Done", command = main)
    btn.pack()
    a.mainloop()
def pickActionAndQuit():
    pass  
top = Tkinter.Tk()
top.wm_title("Image Artist PRO")
A = Tkinter.Button(top, text = "Select One or More Images", command = getImageName, width = 50, height= 10)
B = Tkinter.Button(top, text = "Select File Directory", command = getDirectory, width = 50, height = 10)
C = Tkinter.Button(top, text = "Done", command = pickActionAndQuit, width = 50, height = 10)
A.pack()
B.pack()
C.pack()
